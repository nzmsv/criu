...
/*
 * Restore-side per-QP UHW-pack dispatch.
 *
 * The QP twin of rdma_dispatch_restore_cq_uhw_pack(): core builds the
 * driver-agnostic half of UVERBS_METHOD_RESTORE_QP and delegates the
 * driver-private UHW (rxe: the rxe_restore_qp_req wire state) back to the
 * owning plugin, keyed by @criu_driver against each plugin's
 * cr_rdma_provided_driver -- same reasoning as the CQ pack dispatch (the
 * hook chain alone can't tell an rxe QP from an mlx5 one).
 *
 * Like the CQ pack dispatch we key on provided-driver alone, not on hook
 * presence: a matched plugin that exposes no UHW_PACK hook is a valid
 * "this driver has no per-QP UHW" case and yields an empty @uhw (rc 0),
 * leaving core to issue a UHW-less RESTORE_QP. Hard failures:
 *   - two plugins declare the same provided-driver (-EEXIST).
 *   - no plugin matches (-ENOENT).
 */
int rdma_dispatch_restore_qp_uhw_pack(uint32_t criu_driver, const RdmaUobjEntry *e, struct rdma_uhw_spec *uhw)
{
	...
	list_for_each_entry(this, &cr_plugin_ctl.head, list) {
		...
		if (winner) {
			pr_err("QP restore (ufile_handle=%u): multiple plugins declare cr_rdma_provided_driver=%u "
			       "('%s' and '%s'); operator's plugin set is inconsistent.\n",
			       e && e->has_ufile_handle ? e->ufile_handle : 0, criu_driver, winner_name, this->d->name);
			return -EEXIST;
		}
	...
	if (!winner) {
		pr_err("QP restore (ufile_handle=%u): no loaded RDMA plugin exports cr_rdma_provided_driver=%u; "
		       "cannot reshape per-QP UHW.\n",
		       e && e->has_ufile_handle ? e->ufile_handle : 0, criu_driver);
		return -ENOENT;
	}

	fn = winner->d->hooks[CR_PLUGIN_HOOK__RDMA_RESTORE_UOBJ_QP_UHW_PACK];
	if (!fn) {
		pr_debug("QP restore: plugin '%s' exposes no RESTORE_UOBJ_QP_UHW_PACK; issuing UHW-less RESTORE_QP "
			 "(criu_driver=%u)\n",
			 winner_name, criu_driver);
		return 0;
	}

	pr_debug("QP restore: dispatching RESTORE_UOBJ_QP_UHW_PACK to plugin '%s' (criu_driver=%u ufile_handle=%u)\n",
		 winner_name, criu_driver, e && e->has_ufile_handle ? e->ufile_handle : 0);
	return fn(e, uhw);
}
...
/*
 * Restore-side per-PD UHW-pack dispatch.
 *
 * The PD twin of rdma_dispatch_restore_cq_uhw_pack(): core builds the
 * driver-agnostic half of UVERBS_METHOD_RESTORE_PD (the target handle)
 * and delegates the driver-private UHW (mlx5: the source FW pdn) back to
 * the owning plugin, keyed by @criu_driver against each plugin's
 * cr_rdma_provided_driver.
 *
 * Like the CQ/QP pack dispatch we key on provided-driver alone, not on
 * hook presence: a matched plugin that exposes no UHW_PACK hook is a
 * valid "this driver has no per-PD UHW" case and yields an empty @uhw
 * (rc 0), leaving core to issue a UHW-less RESTORE_PD (rxe). Hard
 * failures:
 *   - two plugins declare the same provided-driver (-EEXIST).
 *   - no plugin matches (-ENOENT).
 */
int rdma_dispatch_restore_pd_uhw_pack(uint32_t criu_driver, const RdmaUobjEntry *e, struct rdma_uhw_spec *uhw)
{
	...
	plugin_desc_t *winner = NULL;
	const char *winner_name = NULL;
	CR_PLUGIN_HOOK__RDMA_RESTORE_UOBJ_PD_UHW_PACK_t *fn;
	...
	list_for_each_entry(this, &cr_plugin_ctl.head, list) {
		...
		if (winner) {
			pr_err("PD restore (ufile_handle=%u): multiple plugins declare cr_rdma_provided_driver=%u "
			       "('%s' and '%s'); operator's plugin set is inconsistent.\n",
			       e && e->has_ufile_handle ? e->ufile_handle : 0, criu_driver, winner_name, this->d->name);
			return -EEXIST;
		}
	...
	if (!winner) {
		pr_err("PD restore (ufile_handle=%u): no loaded RDMA plugin exports cr_rdma_provided_driver=%u; "
		       "cannot reshape per-PD UHW.\n",
		       e && e->has_ufile_handle ? e->ufile_handle : 0, criu_driver);
		return -ENOENT;
	}

	fn = winner->d->hooks[CR_PLUGIN_HOOK__RDMA_RESTORE_UOBJ_PD_UHW_PACK];
	if (!fn) {
		pr_debug("PD restore: plugin '%s' exposes no RESTORE_UOBJ_PD_UHW_PACK; issuing UHW-less RESTORE_PD "
			 "(criu_driver=%u)\n",
			 winner_name, criu_driver);
		return 0;
	}

	pr_debug("PD restore: dispatching RESTORE_UOBJ_PD_UHW_PACK to plugin '%s' (criu_driver=%u ufile_handle=%u)\n",
		 winner_name, criu_driver, e && e->has_ufile_handle ? e->ufile_handle : 0);
	return fn(e, uhw);
}
...
/*
 * Restore-side per-QP timing selector. The QP twin of
 * rdma_dispatch_restore_cq_needs_pie() -- same by-provided-driver keying
 * and same "no hook == master-side default" contract. Asks the owning
 * plugin whether its QPs restore master-side (rxe: the SQ/RQ ring is a
 * kernel-page mmap the pie's VMA pass then maps) or must defer to the pie
 * (mlx5: RESTORE_QP pins the source WQ-ring / doorbell pages from
 * current->mm, so the verb must run after the VMAs are laid out).
 *
 * Return value: > 0 defer to pie, 0 master-side. Hard failures:
 *   - two plugins declare the same provided-driver (-EEXIST).
 *   - no plugin matches (-ENOENT).
 */
int rdma_dispatch_restore_qp_needs_pie(uint32_t criu_driver)
{
	...
	list_for_each_entry(this, &cr_plugin_ctl.head, list) {
		...
		if (winner) {
			pr_err("QP restore timing: multiple plugins declare cr_rdma_provided_driver=%u "
			       "('%s' and '%s'); operator's plugin set is inconsistent.\n",
			       criu_driver, winner_name, this->d->name);
			return -EEXIST;
		}
	...
	if (!winner) {
		pr_err("QP restore timing: no loaded RDMA plugin exports cr_rdma_provided_driver=%u; "
		       "cannot decide restore timing.\n",
		       criu_driver);
		return -ENOENT;
	}

	fn = winner->d->hooks[CR_PLUGIN_HOOK__RDMA_RESTORE_UOBJ_QP_NEEDS_PIE];
	if (!fn) {
		pr_debug("QP restore timing: plugin '%s' exposes no RESTORE_UOBJ_QP_NEEDS_PIE; restoring QP "
			 "master-side (criu_driver=%u)\n",
			 winner_name, criu_driver);
		return 0;
	}
