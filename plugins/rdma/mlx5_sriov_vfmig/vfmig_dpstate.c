 ...
 * rdma_mlx5_vfmig plugin. The dump path drives the firmware
 * RUNNING <-> RUNNING_P2P <-> STOP ladder through these two entry
 * points -- fused today (CHECKPOINT_DEVICES pause, fini resume), and a
 * single ladder edge at a time once the cross-host barrier splits the
 * pause around its rendezvous. Keeping them in one place means every
 * caller opens /dev/mlx5_vfmig/<pf_bdf> the same way and passes the same
 * MLX5_VFMIG_DIR_FLAG_* direction selector.
 *
 * @dir_flags == 0 drives the legacy fused pair (all-or-nothing on
 * failure per the kernel contract); MLX5_VFMIG_DIR_FLAG_INITIATOR /
 * _RESPONDER drive a single ladder edge. Both ioctls are idempotent: a
 * request already satisfied returns 0 with no firmware traffic.
 *
...
int vfmig_dp_suspend(const char *pf_bdf, uint32_t vf_id, uint32_t dir_flags)
{
	...
	close(fd);
	if (rc) {
		pr_perror("vfmig: SUSPEND_VHCA(pf=%s vf_id=%u flags=0x%x)", pf_bdf, vf_id, dir_flags);
		return -1;
	}
	return 0;
}
...
int vfmig_dp_resume(const char *pf_bdf, uint32_t vf_id, uint32_t dir_flags)
{
	...
	close(fd);
	if (rc) {
		pr_perror("vfmig: RESUME_VHCA(pf=%s vf_id=%u flags=0x%x)", pf_bdf, vf_id, dir_flags);
		return -1;
	}
	return 0;
}
