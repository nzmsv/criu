...
#
# DMA-BUF variant of run_vfmig_mr_cr.sh. The holder registers its MR over a
# udmabuf wrapping a sealed memfd (HOLDER_ALLOC_DMABUF_MR=1) instead of an
...
# registers a persistent MR over a known-pattern buffer (HOLDER_ALLOC_DMABUF_MR=1,
...
grep -qaE 'READY .* pd=[0-9]+ mr_lkey=0x[0-9a-f]+ mr_rkey=0x[0-9a-f]+' "$HLOG" \
	|| { cat "$HLOG" >&2; die "holder did not register an MR (HOLDER_ALLOC_DMABUF_MR not honoured?)"; }
